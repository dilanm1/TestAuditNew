...
{
    public class SavedExaminationTC
    {
        ...
        public SavedExaminationTC(int mid, int sid, String sname, String location)
        {
            this.mid = mid;
            this.sid = sid;
            this.sname = sname;
        ...
        public void GetSavedQuestions()
        {
            if (location == "TECOM Investments Corporate")
            {
                ...
                cmd.CommandText = "SELECT tblQuestionsTcomCorp.qid,tblQuestionsTcomCorp.question,tblQuestionsTcomCorp.ref,tblQuestionsTcomCorp.guid, ISNULL(tblSavedAnswersAuditor.mid,0) as mid, ISNULL(tblSavedAnswersAuditor.answer,0) as answer,ISNULL(tblSavedAnswersAuditor.evidence,0) as evidence,ISNULL(tblSavedAnswersAuditor.comments,0) as comments,ISNULL(tblSavedAnswersAuditor.action,0) as action, tblQuestionsTcomCorp.sid FROM  tblQuestionsTcomCorp LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsTcomCorp.qid = tblSavedAnswersAuditor.qid and (@mid IS NULL OR tblSavedAnswersAuditor.mid = @mid) and (@loc IS NULL OR tblSavedAnswersAuditor.location=@loc) WHERE tblQuestionsTcomCorp.sid=@sid ORDER BY tblQuestionsTcomCorp.id";
            ...
            else
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "SELECT tblQuestionsTComSBU.qid,tblQuestionsTComSBU.question,tblQuestionsTComSBU.ref,tblQuestionsTComSBU.guid, ISNULL(tblSavedAnswersAuditor.mid,0) as mid, ISNULL(tblSavedAnswersAuditor.answer,0) as answer,ISNULL(tblSavedAnswersAuditor.evidence,0) as evidence,ISNULL(tblSavedAnswersAuditor.comments,0) as comments,ISNULL(tblSavedAnswersAuditor.action,0) as action, tblQuestionsTComSBU.sid FROM  tblQuestionsTComSBU LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsTComSBU.qid = tblSavedAnswersAuditor.qid and (@mid IS NULL OR tblSavedAnswersAuditor.mid = @mid) and (@loc IS NULL OR tblSavedAnswersAuditor.location=@loc) WHERE tblQuestionsTComSBU.sid=@sid ORDER BY tblQuestionsTComSBU.id";
                cmd.Parameters.Add("@mid", SqlDbType.Int);
                cmd.Parameters.Add("@loc", SqlDbType.NVarChar);
                cmd.Parameters.Add("@sid", SqlDbType.Int);
