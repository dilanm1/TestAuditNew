...
namespace AuditMgtNew.Old_App_Code
{
    public class QuestionsSavedDP  
    {
        ...
        public QuestionsSavedDP(int mid, int sid, String sname, String location)
        {
            this.mid = mid;
            this.sid = sid;
            this.sname = sname;
        ...
        public void GetSavedQuestions()
        {
            if (location == "Dubai Property Group Corporate")
            {
                ...
                cmd.CommandText = "SELECT DISTINCT tblQuestionsDPSubCorp.id,tblQuestionsDPSubCorp.qid, tblQuestionsDPSubCorp.question, tblQuestionsDPSubCorp.ref, tblQuestionsDPSubCorp.guid, ISNULL(tblSavedAnswersAuditor.answer, 0) as answer,ISNULL(tblSavedAnswersAuditor.evidence, 0) as evidence,ISNULL(tblSavedAnswersAuditor.comments, 0) as comments, tblQuestionsDPSubCorp.sid FROM tblQuestionsDPSubCorp LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsDPSubCorp.qid = tblSavedAnswersAuditor.qid WHERE (tblQuestionsDPSubCorp.sid =@sid)ORDER BY tblQuestionsDPSubCorp.id";
            ...
            else
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "SELECT DISTINCT tblQuestionsDPSub.id,tblQuestionsDPSub.qid, tblQuestionsDPSub.question, tblQuestionsDPSub.ref, tblQuestionsDPSub.guid, ISNULL(tblSavedAnswersAuditor.answer, 0) as answer,ISNULL(tblSavedAnswersAuditor.evidence, 0) as evidence,ISNULL(tblSavedAnswersAuditor.comments, 0) as comments, tblQuestionsDPSub.sid FROM tblQuestionsDPSub LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsDPSub.qid = tblSavedAnswersAuditor.qid WHERE (tblQuestionsDPSub.sid =@sid)ORDER BY tblQuestionsDPSub.id";
                cmd.Parameters.Add("@sid", SqlDbType.Int);
                cmd.Parameters["@sid"].Value = Sid;
                //SqlDataAdapter da = new SqlDataAdapter("select id,sid,qid,question,answer,evidence,comments from tblSavedAnswersAuditor where sid =" + sid, con);
                // SqlDataAdapter da = new SqlDataAdapter("SELECT DISTINCT tblQuestionsJMSub.id,tblQuestionsJMSub.qid, tblQuestionsJMSub.question, tblQuestionsJMSub.ref, tblQuestionsJMSub.guid, ISNULL(tblSavedAnswersAuditor.answer, 0) as answer,ISNULL(tblSavedAnswersAuditor.evidence, 0) as evidence,ISNULL(tblSavedAnswersAuditor.comments, 0) as comments, tblQuestionsJMSub.sid FROM tblQuestionsJMSub LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsJMSub.qid = tblSavedAnswersAuditor.qid WHERE (tblQuestionsJMSub.sid =@sid)ORDER BY tblQuestionsJMSub.id", con); 
