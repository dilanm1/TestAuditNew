...
{
    public class SavedExaminationDP
    {
    public int SIZE;
        public int mid;
        public int sid;
        ...
        public SavedExaminationDP(int mid, int sid, String sname, String location)
        {
            this.mid = mid;
            this.sid = sid;
            this.sname = sname;
        ...
        public void GetSavedQuestions()
        {
            if (location == "Dubai Property Group Corporate")
            {
                ...
                cmd.CommandText = "SELECT tblQuestionsDPSubCorp.qid,tblQuestionsDPSubCorp.question,tblQuestionsDPSubCorp.ref,tblQuestionsDPSubCorp.guid, ISNULL(tblSavedAnswersAuditor.mid,0) as mid, ISNULL(tblSavedAnswersAuditor.answer,0) as answer,ISNULL(tblSavedAnswersAuditor.evidence,0) as evidence,ISNULL(tblSavedAnswersAuditor.comments,0) as comments,ISNULL(tblSavedAnswersAuditor.action,0) as action, tblQuestionsDPSubCorp.sid FROM  tblQuestionsDPSubCorp LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsDPSubCorp.qid = tblSavedAnswersAuditor.qid and (@mid IS NULL OR tblSavedAnswersAuditor.mid = @mid) and (@loc IS NULL OR tblSavedAnswersAuditor.location=@loc) WHERE tblQuestionsDPSubCorp.sid=@sid ORDER BY tblQuestionsDPSubCorp.id";
            ...
            else
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = cmd.CommandText = "SELECT tblQuestionsDPSub.qid,tblQuestionsDPSub.question,tblQuestionsDPSub.ref,tblQuestionsDPSub.guid, ISNULL(tblSavedAnswersAuditor.mid,0) as mid, ISNULL(tblSavedAnswersAuditor.answer,0) as answer,ISNULL(tblSavedAnswersAuditor.evidence,0) as evidence,ISNULL(tblSavedAnswersAuditor.comments,0) as comments,ISNULL(tblSavedAnswersAuditor.action,0) as action,tblQuestionsDPSub.sid FROM  tblQuestionsDPSub LEFT JOIN tblSavedAnswersAuditor ON tblQuestionsDPSub.qid = tblSavedAnswersAuditor.qid and (@mid IS NULL OR tblSavedAnswersAuditor.mid = @mid) and (@loc IS NULL OR tblSavedAnswersAuditor.location=@loc) WHERE tblQuestionsDPSub.sid=@sid ORDER BY tblQuestionsDPSub.id";
                cmd.Parameters.Add("@mid", SqlDbType.Int);
                cmd.Parameters.Add("@loc", SqlDbType.NVarChar);
                cmd.Parameters.Add("@sid", SqlDbType.Int);
