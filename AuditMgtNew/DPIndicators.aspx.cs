...
{
    public partial class DPIndicators : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
          ...
          //  Session["location"] = null;
            if (!IsPostBack)
            {
                if ((Session["location"]) == null || (Session["location"].ToString()) == "Dubai Property Group Corporate")
                {

                    // BindEmployeeDetails();
                    DropDownList1.Visible = true;
                    string query = ("SELECT tbllocation.location, tbllocation.locationid, tbllocation.verticalid FROM tblBuilding INNER JOIN tbllocation ON tblBuilding.locationid = tbllocation.locationid WHERE (tbllocation.verticalid = 2) AND tbllocation.locationid !=14 ");
