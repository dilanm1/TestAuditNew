...
{
    public partial class AuditSavedDP : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (this.Page.PreviousPage != null)
            {
                ...
                SavedExaminationDP exam = new SavedExaminationDP(Int32.Parse(Session["mid"].ToString()), Int32.Parse(sid), sname, location);
                exam.GetSavedQuestions();
                Session.Add("questionsSavedd", exam);
        ...
        public void DisplayQuestion()
        {
            ...
            SavedExaminationDP e = (SavedExaminationDP)Session["questionsSavedd"];
            // display data
            ...
            SavedQuestions q = e.questions[e.curpos];
            lblQid.Text = q.qid;
            ...
            if (q.evidence != "0")
            {
                TextBox2.Text = q.evidence;
            }
            else if (q.question == "0")
            {
                TextBox2.Text = "";
            }
            if (q.comments != "0")
            {
                ...
                TextBox3.Text = q.comments;
            }
            else if (q.comments == "0")
            {
                TextBox3.Text = "";
            }
            if (q.action != "0")
            {
                RadioButtonList1.Text = q.action;
            }
            else if (q.action == "0")
            {
                RadioButtonList1.ClearSelection();
            }
            // Label3.Text = q.answer.ToString();
            //TextBox4.Text = q.guid;
        ...
        {
            SavedExaminationDP exam = (SavedExaminationDP)Session["questionsSavedd"];
            SavedQuestions q = exam.questions[exam.curpos];
        ...
        protected void btnNext_ServerClick(object sender, EventArgs e)
        {
            ...
            SavedQuestions q2 = exam.questions[exam.curpos];
            ...
            {
                foreach (SavedQuestions q in exam.questions)
                {
                    ...
                    cmd4.CommandText = "Delete from tblSavedAnswersAuditor where examid=@examid and locationid=@locationid and mid =@mid and sid = @sid";
                ...
                SavedQuestions q3 = exam.questions[exam.curpos];
                ...
                foreach (SavedQuestions q1 in exam.questions)
                {
                    //if (q.IsCorrect())
                    //    cnt++;
                    cnt = cnt + q1.answer;
                ...
                SqlCommand cmd3 = new SqlCommand("UPDATE oe_subjects_status SET score = @score,status =@status  where location=@loc and sid=@sid and mid =@mid", con6);
                ...
                SavedQuestions q6 = exam.questions[exam.curpos];
            ...
            else
            {
                ...
                TextBox3.Text = "";
                DisplayQuestion();
                if (rbAns1.Checked == true && TextBox2.Text == "" && TextBox3.Text == "")
                {
                    rbAns1.Checked = false;
                }
            }
        ...
        protected void ImageButton3_ServerClick(object sender, EventArgs e)
        {
            ...
            Session.Remove("questionsSavedd");
            string context = Session["location"].ToString();
        ...
        protected void ImageButton8_ServerClick(object sender, EventArgs e)
        {
            ...
            for (var questionPos = 0; questionPos <= exam.curpos; questionPos++)	// <--- Loop construct changed.  Check saves ALL questions.
            {
                //Examination exam = (Examination)Session["questions"];
                //Examination exam = (Examination)Session["questions"];
                SavedQuestions q = exam.questions[questionPos];
                SqlConnection con = new SqlConnection(DBUtil.ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("select isnull( max(examid),0) + 1 from oe_exams", con);
                int examid = (Int32)cmd.ExecuteScalar();
                cmd.CommandText = ("select locationid from tbllocation where location ='" + exam.location + "'");
                int locationid2 = (Int32)cmd.ExecuteScalar();
                ...
                cmd.CommandText = @"
				        ...
				        WHEN MATCHED THEN
					        UPDATE SET t.answer = @answer, t.evidence = @evidence, t.comments = @comments
				        WHEN NOT MATCHED THEN
					        INSERT ( mid, examid, sid, locationid, location, qid, question, answer, evidence, comments)
					        VALUES (@mid,@examid,@sid,@locationid,@location,@qid,@question,@answer,@evidence,@comments);
			        ";

                cmd.Parameters.Add("@examid", SqlDbType.Int).Value = examid;
                cmd.Parameters.Add("@locationid", SqlDbType.NVarChar).Value = locationid2;
                cmd.Parameters.Add("@location", SqlDbType.NVarChar).Value = exam.location;
                cmd.Parameters.Add("@mid", SqlDbType.Int).Value = exam.mid;
                cmd.Parameters.Add("@sid", SqlDbType.Int).Value = exam.sid;
                cmd.Parameters.Add("@qid", SqlDbType.NVarChar).Value = q.qid;
            ...
            cmds2.Parameters.Add("@date", SqlDbType.DateTime).Value = DateTime.Today.ToString();
            // cmd2.Parameters.Add("@score", SqlDbType.Float).Value = exam.score;
